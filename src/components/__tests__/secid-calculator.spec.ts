...
describe('secid-calculator.vue', () => {
    ...
    it('should toggle the Blue Burst checkbox', async () => {
        ...
        const blueBurstCheckboxElement = blueBurstCheckbox.element as HTMLInputElement

        expect(blueBurstCheckboxElement.checked).toBe(true)
        await blueBurstCheckbox.trigger('click')
        expect(blueBurstCheckboxElement.checked).toBe(false)
    })
})
